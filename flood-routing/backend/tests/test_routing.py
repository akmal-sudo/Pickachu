...
def test_nearest_edge_snaps_click_to_correct_road():
    """
    Used by the click-to-flood-a-chosen-road feature: a click near a given
    road segment should resolve to that segment's (u, v, k), not a
    different one, even when multiple roads are nearby.
    """
    G = nx.MultiDiGraph()
    G.graph["crs"] = "epsg:4326"
    G.add_node(1, x=0.000, y=0.000)
    G.add_node(2, x=0.001, y=0.000)  # east of 1 -> horizontal road
    G.add_node(3, x=0.001, y=0.001)  # north of 2 -> vertical road
    G.add_edge(1, 2, key=0, travel_time=10, length=100)
    G.add_edge(2, 3, key=0, travel_time=10, length=100)

    engine = RoutingEngine("test-area")
    engine.G = G

    assert engine.nearest_edge(0.0000, 0.0005) == (1, 2, 0)
    assert engine.nearest_edge(0.0005, 0.0010) == (2, 3, 0)
